...
- 固定 JSON 结构生成并严格校验真实日期、星期、整数时长、课程重叠和资源 URL。
- 本地强制检查生成日期位于选择范围内，开始时间属于填写的“周几 HH:mm”可用时段。
- 数学主题补充中国大学 MOOC 主课；通过 Bing RSS 为每节课提取相关的可信资源，并保留 B 站与必应检索入口作为兜底。
- 选择 PNG、JPG、WEBP 或 PDF 课表，调用视觉模型识别固定课程。
- PDF 在本机最多渲染前 4 页为图片，再发送到用户指定的视觉模型。
- 将识别出的占用时段加入生成约束，并在导出前进行确定性的时间重叠检查。
- API 请求超过 120 秒会给出错误；资源检索超过 10 秒会自动使用固定兜底链接。
...
便携版输出到 `release/ai-schedule-generator-0.1.1.exe`。
